...
func (e *EchoCommand) Execute() {
	...
	if strings.HasPrefix(echoArgument, "'") && strings.HasSuffix(echoArgument, "'") {
		util.PrintOutput(strings.ReplaceAll(echoArgument, "'", ""))
		return
	}
	if strings.HasPrefix(echoArgument, "\"") && strings.HasSuffix(echoArgument, "\"") {
		util.PrintOutput(processDoubleQuotedStrings(echoArgument))
		return
	}

	echoPhrases := util.SplitInput(echoArgument, " ")
	...
	util.PrintOutput(strings.Join(echoPhrases, " "))
}

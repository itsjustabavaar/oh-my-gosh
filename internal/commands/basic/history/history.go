...
func (h *HistoryCommand) Execute() {
	components := util.SplitInput(h.Input, " ")

	if len(components) > 1 && components[1] == "clean" {
		cleaningErr := cleanHistory()
		if cleaningErr != nil {
			util.PrintError(historyCommand, errorutil.ErrCleaningHistory)
			return
		}
	...
	if vars.CurrentUser.Username == "" {
		...
		if err != nil {
			util.PrintError(historyCommand, err)
			return
		}
	} else {
...
func GetUserCommandsSummary() ([]CommandSummary, error) {
	...
	err = db.Model(&models.CommandHistory{}).
...
func cleanHistory() error {
	...
	_ = db.Where("user_id = ?", vars.CurrentUser.ID).Delete(&models.CommandHistory{})
